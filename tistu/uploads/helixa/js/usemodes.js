...
(function () {
  ...
  const traders = {
    ...
    draw() {
      ...
      ctx.beginPath(); ctx.arc(lx, ly, 4, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
    }
  };
  ...
  const builders = {
    ...
    init() {
      ...
      const ox = 0.5 - totalW / 2, oy = 0.40 - totalH / 2;   // biased toward the upper region
    ...
    draw() {
      // assemble progress per module (slower, calmer)
      ...
      // structural links between settled neighbours (quiet connective tissue)
      ...
      // modules — translucent blueprint tiles, not solid blocks
      for (const m of this.mods) {
        ...
        // inner slot line + node (the "component")
        ...
        ctx.beginPath(); ctx.arc(x + w * 0.82, y + h * 0.36, Math.max(1.4, w * 0.04), 0, Math.PI * 2); ctx.fill();
      }
      ctx.globalAlpha = 1;
    }
  };
  ...
  const treasuries = {
    ...
    draw() {
      ...
      // slow concentric rings (measured breathing)
      for (let i = 0; i < 3; i++) {
      ...
      // central heavy mass with slow vertical settle
      ...
      ctx.fillStyle = mg; ctx.beginPath(); ctx.arc(cx, cy + bob, W * 0.075, 0, Math.PI * 2); ctx.fill();
      // a measured equilibrium line
      ...
      // heavy ballast bars settling slowly toward an upper-middle base
  ...
  let mode = 'traders';
  let switchT = 1; // 0..1 fade-in after a switch
  modes[mode].init();
  ...
  function frame(now) {
    ...
    // faint shared grid (visual family)
    ctx.globalAlpha = 1;
