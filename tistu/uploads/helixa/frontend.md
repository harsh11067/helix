...
### 3.2 How Color is Used

**Backgrounds are deep void.** The void is the canvas on which life appears. It should feel like staring into space or deep ocean.

**Cream is the default foreground.** Pure white is too clinical. Cream is warm, editorial, comfortable for long reading sessions.

**Aqua is reserved for life.** Living strategies pulse aqua. Active processes glow aqua. The DNA helix is rendered in aqua gradients. Do not overuse — aqua should always signal *something is alive here*.

**Amber is reserved for value.** Gains, fees earned, top-of-leaderboard. Amber + aqua together when a strategy is both alive AND profitable.

**Coral is reserved for danger.** Drawdowns, tolerance breaches, dying organisms. Should appear sparingly so it actually carries weight.

### 3.3 Gradients (Use Sparingly)

```css
/* The Genesis Gradient — for hero moments, primary CTAs */
--gradient-genesis: linear-gradient(
  135deg,
  var(--aqua) 0%,
  var(--aqua-bright) 50%,
  var(--amber) 100%
...
/* The Lineage Gradient — for edges in the tree, lineage indicators */
--gradient-lineage: linear-gradient(
  180deg,
  var(--aqua-glow) 0%,
  transparent 100%
);

/* The Void Gradient — for atmospheric depth, page backgrounds */
--gradient-void: radial-gradient(
  ellipse at 50% 0%,
  rgba(77, 219, 224, 0.04) 0%,
  transparent 70%
);
```
...
- Subtle DNA helix animation in background (rotating slowly, very low opacity)
